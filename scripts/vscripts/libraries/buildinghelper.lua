...
function BuildingHelper:OnTreeCut(keys)
    ...
    tree.chopped_dummy = CreateUnitByName("dummy_unit", treePos, false, nil, nil, 0)
...
function BuildingHelper:PlaceBuilding(player, name, location, construction_size, pathing_size, angle)
    ...
    local player = PlayerResource:GetPlayer(playerID)
    local playersHero = player.fake_omniknight_hero -- need to use omniknight as the hero otherwise game gets confused when the player chooses a hero class
...
function BuildingHelper:CreatePedestalForBuilding(entity, buildingName, location, pedestalName)
    ...
    --prop:SetForwardVector(Vector(1,1,0)) -- for turning pedestal
