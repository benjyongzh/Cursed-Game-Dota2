modifier_vampire_impact_lua_movement = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_vampire_impact_lua_movement:IsHidden()
	return true
end

function modifier_vampire_impact_lua_movement:IsPurgable()
	return false
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_vampire_impact_lua_movement:OnCreated( kv )
	if IsServer() then
		...
		local abil = self:GetParent():FindAbilityByName("vampire_ascend_lua")
...
function modifier_vampire_impact_lua_movement:OnRefresh( kv )	
end

function modifier_vampire_impact_lua_movement:OnDestroy( kv )
	if IsServer() then
        ...
        local enemies = FindUnitsInRadius(
			...
			caster:GetAbsOrigin(),
			nil,
			self:GetAbility():GetSpecialValueFor( "impact_radius" ),
		...
		local trees = GridNav:GetAllTreesAroundPoint( caster:GetAbsOrigin(), self:GetAbility():GetSpecialValueFor( "impact_radius" )/2, true )
...
--------------------------------------------------------------------------------
-- Motion effects
function modifier_vampire_impact_lua_movement:SyncTime( iDir, dt )
...
function modifier_vampire_impact_lua_movement:UpdateHorizontalMotion( me, dt )
	self:SyncTime(1, dt)
...
function modifier_vampire_impact_lua_movement:OnHorizontalMotionInterrupted()
	if IsServer() then
		self:Destroy()
	end
end

function modifier_vampire_impact_lua_movement:UpdateVerticalMotion( me, dt )
	self:SyncTime(2, dt)
	local parent = self:GetParent()
...
function modifier_vampire_impact_lua_movement:OnVerticalMotionInterrupted()
	if IsServer() then
		self:Destroy()
	end
end

function modifier_vampire_impact_lua_movement:GetActivityTranslationModifiers()
	return "hunter_night"
end